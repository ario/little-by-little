...
export async function demoVoice(body){if(body.child==='neev'&&typeof body.ui_text==='string')return {text:body.ui_text};const data=demoCalendar(),e=data.children.neev.find(e=>e.id===body.event);return {text:e?`${e.title}. ${label(bounds(e)[0])}. ${until(e,data.today).text}.`:`${label(body.date)}.`};}
