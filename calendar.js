...
function build(profile){const section=node('section',`child calendar-child ${profile.theme}`);section.dataset.calendarChild=profile.id;section.setAttribute('aria-label',`${profile.name}’s calendar`);
 ...
 const home=button('Today','today','calendar-today');home.setAttribute('aria-label',`${profile.name}: Go to today`);head.append(home);
 const tools=node('nav','calendar-controls');tools.setAttribute('aria-label',`${profile.name} calendar view`);const zoom=node('div','calendar-zoom');for(const kind of ['day','week','month']){const b=button(kind[0].toUpperCase()+kind.slice(1),'zoom');b.dataset.zoom=kind;zoom.append(b);}tools.append(zoom);
...
function day(view,content){const layout=node('div','calendar-day'),anchor=node('div','date-anchor');anchor.append(node('span','date-relative',dayDistance(view.day,selectedToday())),node('h3','',label(view.day,{weekday:'long'})),node('strong','date-number',String(Number(view.day.slice(8)))),node('span','date-month',label(view.day,{month:'long',year:'numeric'})));
 if(view.id==='neev'){const hear=button('Hear this day','hear','hear-day');anchor.append(hear);}layout.append(anchor);
...
function detail(view,content){const e=view.detail,box=node('article','calendar-detail');box.append(button('‹ Back to calendar','close','detail-close'));box.append(node('div','detail-picture',pict(e.title)),node('h3','detail-title',e.title));const count=until(e,selectedToday());box.append(node('p','detail-countdown',count.text));if(count.n>0&&count.n<=14)box.append(node('div','sleep-moons','☾ '.repeat(count.n).trim()));if(count.n>=14){const weeks=Math.floor(count.n/7),days=count.n%7;box.append(node('p','sleep-weeks',`${weeks} weeks${days?` and ${days} ${days===1?'day':'days'}`:''}`));}box.append(node('p','detail-date',label(bounds(e)[0])),node('p','detail-time',eventTime(e)));if(e.location)box.append(node('p','detail-location',`📍 ${e.location}`));if(e.description)box.append(node('p','detail-description',e.description));if(view.id==='neev')box.append(button('Hear it again','hear','hear-day'));content.append(box);}
